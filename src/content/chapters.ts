        ...
        blocks: [
          {
            ...
            rows: [
              ["1", "8h", "LLM、消息、token、结构化输出", "三个模型调用脚本；记录延迟与 token"],
              ["2", "8h", "Prompt 边界与 Zod", "把一个非结构化任务改成类型化输出并单测"],
              ["3", "10h", "Tool Calling", "天气、订单两个只读 Tool；非法参数测试"],
              ["4", "10h", "ReAct 与副作用", "天气邮件 Agent；Dry Run 与确认流程"],
              ["5", "10h", "Embedding、切块、Qdrant", "导入 10 篇文档，制作 20 问检索集"],
              ["6", "10h", "RAG 质量", "比较三组 chunk/topK，记录 Recall@K"],
              ["7", "8h", "短时与长期记忆", "thread 隔离、摘要、Postgres 恢复"],
              ["8", "10h", "LangGraph", "把隐式循环改成可视状态图并设置终止条件"],
              ["9", "10h", "多 Agent 与 HITL", "Supervisor + 两个最小权限 Worker"],
              ["10", "12h", "API、流式 UI、安全", "Hono API、状态 UI、鉴权限流方案"],
              ["11", "12h", "评估、追踪、成本", "30 条 Agent eval；输出失败分类报告"],
              ["12", "14h", "最终项目与发布", "README、演示、部署、架构复盘与公开仓库"],
            ],
          },
          {
            type: "callout",
        ...
        blocks: [
          {
            ...
            headers: ["周", "方向", "建议产出"],
            rows: [
              ["13", "MCP", "把一个内部 REST API 封装为权限受控的 MCP Server"],
              ["14", "语音与多模态", "支持图片/PDF输入或实时语音的 Agent 原型"],
              ["15", "本地/开源模型", "同一评估集比较云模型与本地模型的质量、延迟、成本"],
              ["16", "开源贡献", "为 LangChain.js、LangGraph.js 或相关集成提交 PR"],
            ],
          },
        ],
      },
