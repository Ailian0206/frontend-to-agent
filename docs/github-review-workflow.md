...
本文档定义 `frontend-to-agent` 的分支、PR、CI、独立审核与合并规则。目标是在保留小型维护效率的同时，让里程碑变更有稳定、可追溯的审核证据。
...
2. Codex 必须读取完整评论并技术核验，不盲目接受 finding。
3. 成立的问题在原 PR 分支最小修复，重新运行聚焦测试和完整门禁后提交、推送。
4. Claude 成功发布审核后禁止再次调用；修复正确性由 Codex 的复现、本地门禁和最终 CI 保证。
5. 首次命令失败或没有发布评论时最多重试一次；一旦产生有效审核评论，该 PR 的审核额度即已使用。

Cursor Bugbot 或其他自动评论只能作为附加反馈，不能替代这次独立审核，也不是合并门禁。
...
禁止 squash、rebase、amend、force push。当前 PR 完成审核、CI 与合并闭环前，不开始下一个里程碑 PR。
