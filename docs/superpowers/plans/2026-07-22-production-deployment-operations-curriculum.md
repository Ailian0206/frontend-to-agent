...
- [x] **Step 5: 验证 GREEN、图片存在和隐私扫描并提交**
...
- [x] **Step 1: 运行平台完整性测试确认 Supabase RED**

Run: `PATH=$HOME/.nvm/versions/node/v22.22.1/bin:$PATH npm test -- src/content/chapters.test.ts`

Expected: FAIL，P05/P06 尚未装配。

- [x] **Step 2: 只读采集并脱敏 6 张截图**
...
- [x] **Step 1: 运行平台完整性测试确认 Inngest RED**
...
- [x] **Step 1: 运行平台完整性测试确认 Sentry RED**
...
验收记录（2026-07-22）：根项目 Vitest 27 项、示例工程 139 项、Playwright 双视口 26 项（桌面 13、移动 13）全部通过；`GITHUB_PAGES=true npm run build` 成功导出专题深链和 24 张 WebP。P01、P03、P06、P08、P10、P12、P14 在 390×844、1024×768、1440×1000 共 21 个视口组合中无横向溢出、裁切或重叠，24 张懒加载图片滚动进入视口后均为非空像素。最终隐私扫描未发现密钥、邮箱、原始截图或生产数据；`service_role` 仅作为课程术语出现。
